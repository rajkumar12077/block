...
const DashboardColdStorage: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    console.log('Using userName from localStorage:', userName || 'not found');
    
    fetchColdStorageOrders(userId);
    fetchLogisticsUsers();
    fetchTemperatureData(userName || undefined);
  }, []);
  ...
  const handleDispatchOrder = async () => {
    ...
    try {
      ...
      console.log('=== COLD STORAGE DISPATCH DEBUG ===');
      console.log('Dispatching order:', selectedOrderId);
      console.log('To logistics:', selectedLogistics);
      console.log('Logistics name:', selectedLogisticsUser?.name);
      console.log('=== END DEBUG ===');
      
      const response = await axios.put(`/order/cold-storage/dispatch/${selectedOrderId}`, {
    ...
    try {
      ...
      if (coldStorageName) {
        ...
        if (storageResponse.data && storageResponse.data.length > 0) {
          ...
          if (filteredData.length > 0) {
            ...
            setError(`No temperature data found for cold storage name "${coldStorageName}".`);
          }
      ...
      if (response.data) {
        ...
        if (hasData) {
          ...
          setTemperatureData({});
          setError('No temperature data found for this cold storage.');
        }
      } else {
  ...
  return (
    <Box sx={{ p: 3 }}>
          ...
          <FormControl variant="outlined" size="small" sx={{ width: 200 }}>
            ...
            <Select
              ...
              onChange={(e) => {
                ...
                const userName = localStorage.getItem('userName');
                fetchTemperatureData(userName || undefined, value);
              }}
      ...
      {Object.keys(temperatureData).length > 0 && (() => {
        ...
        const userName = localStorage.getItem('userName') || 'cold';
